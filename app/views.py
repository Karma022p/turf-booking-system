...
def player_dashboard(request):
    ...
    if request.method == 'POST':
        ...
        # Get selected players (passed as a JSON string)
        selected_players_json = request.POST.get('selected_players_json', '[]')
        selected_players = json.loads(selected_players_json)
...
def fetch_players(request):
    if request.method == 'GET':
        ...
        # Query players in the same pincode
        players = Player_Details.objects.filter(pincode=pincode).values('id', 'name', 'email', 'phone')
...
def feedback_view(request):
    if request.method == 'POST':
        sports = request.POST.get('sportsSelect')
        feedback_text = request.POST.get('feedbackTextarea').strip()  # Clean whitespace

        # Simple rating assignment based on keywords
        bad_keywords = ['bad', 'poor', 'terrible', 'awful', 'not good', 'disappoint']
        good_keywords = ['good', 'great', 'excellent', 'awesome', 'fantastic', 'happy']
        
        # Convert to lowercase for case-insensitive check
        feedback_lower = feedback_text.lower()
        
        # Check for keywords and assign rating
        if any(keyword in feedback_lower for keyword in bad_keywords):
            rating = 2
        elif any(keyword in feedback_lower for keyword in good_keywords):
            rating = 7
        else:
            rating = 5  # Neutral rating if no keywords matched

        # Save feedback with manual rating
        feedback = Feedback(
            sports=sports,
...
def get_players_by_pincode(request):
    pincode = request.GET.get('pincode', '')
    players = Player_Details.objects.filter(pincode=pincode).values('id', 'name', 'email', 'address', 'city', 'state', 'country', 'pincode')
